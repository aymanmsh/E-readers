				    ...
				    <tbody>
				        ...
				        @foreach( $categories as $category )
				        	<tr>
				        		...
				        		<td>{{$category->lang}}</td>
				        		<td class="settings">
				        			<a class="btn btn-primary" href="{{route('category.restore', ['id' => $category->id])}}"><i class="fa fa-check"></i></a>
				        			<a class="btn btn-danger restore-category" data-id="{{$category->id}}"><i class="fa fa-trash"></i></a>
				        		</td>
				        	</tr>
						@endforeach				       				    	
				    </tbody>
...
@section('script')
	...
	<script type="text/javascript">
		$('.restore-category').on('click', function(event) {
			event.preventDefault();
			var id = $(this).data('id');
			console.log(id);
			Swal.fire({
			title: '{{trans('categories.messages.restore_confirmation')}}',
			text: "{{trans('categories.messages.restore_alert')}}",
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
			confirmButtonText: '{{trans('categories.messages.continue_restore')}}'
		}).then((result) => {
		if (result.value) {
			window.location = '{{route("category.restore")}}/' + id
			Swal.fire(
				'{{trans('categories.messages.restore_done')}}',
				'{{trans('categories.messages.restore_message')}}',
				'success'
			)
		}
		})
