					...
					<div class="row">
						<div class="col-6">
							<div class="form-group">
								...
								<input type="text" class="form-control" name="name" value="{{app('request')->input('name')}}" placeholder="Library Name">
							</div>
						</div>
					</div>
				    ...
				    <tbody>
						...
						@foreach( $libraries as $library )
				        	<tr>
				        		...
				        		<td>{{$library->phone}}</td>
				        		<td class="settings">
				        			<a class="btn btn-primary" href="{{route('library.restore', ['id' => $library->id])}}"><i class="fa fa-check"></i></a>
				        			<a class="btn btn-danger restore-library" data-id="{{$library->id}}"><i class="fa fa-trash"></i></a>
				        		</td>
				        	</tr>
						@endforeach				       				    	
				    </tbody>
...
@section('script')
	...
	<script type="text/javascript">
		$('.restore-library').on('click', function(event) {
		...
		if (result.value) {
			window.location = '{{route("library.restore")}}/' + id
			Swal.fire(
				'Deleted!',
				'Your file has been Rstored.',
				'success'
			)
		}
		})
